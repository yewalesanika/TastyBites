...
function signup() {
    ...
    const submit = (e) => {
        e.preventDefault();
        axios.post("http://localhost:8080/signup", formState)
            .then(() => {
                navigate("/listings");
            })
            .catch((err) => {
    ...
    return (
        <>
            <NavBar></NavBar>
            <h3 className="col-6 offset-3 mt-3">Sign Up for Wanderlust</h3>
            <div className="row mt-3 mb-3">
                <div className="col-6 offset-3">
                    <form onSubmit={submit} noValidate className="needs-validation">
                        <div className="">
                            <label htmlFor="email" className="form-label">Email</label>
                            <input type="email" required className="form-control" name="email"  onChange={(e) => dispatch({ type: "SET_EMAIL", payload: (e.target.value) })}/>
